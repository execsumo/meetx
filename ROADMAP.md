...
- **Fix dictation hotkey reuse.** Dictation works on first hotkey activation but fails on 2nd and subsequent attempts. Also ensure streaming is truly real-time (incremental text injection) rather than batch-mode (all text appears at once when hotkey is released).
