...
- **App-audio self-test** — The tap is verified 2 seconds after recording starts. On silence, the tap chain is rebuilt once with fresh CoreAudio helper enumeration. Persistent silence marks the tap as failed and falls back to mic-only capture; the menu bar shows "Recording (mic only)" so the user is never silently missing remote audio.
...
- **Mic-bleed deduplication** — Before diarization, `SegmentDeduplicator.dropMicBleed` checks each mic segment against temporally overlapping app-track segments. Any mic segment whose words are ≥ 70% covered by a matching app segment (and share ≥ 3 words) is removed. This prevents remote audio from echoing back through the laptop mic and appearing as a phantom second speaker in the transcript.
...
- **Speaker naming window** — When unmatched speakers remain, a dedicated "Name Speakers" window opens with a **playable audio clip** (~10 s of each speaker's clearest speech, extracted from the 48 kHz recording with silence gaps removed so playback is continuous speech), a text field pre-populated with any roster suggestion, and a 120 s auto-dismiss countdown.
- **Custom vocabulary boosting** — CTC-based keyword boosting via `Parakeet CTC 110M` applied to both meeting transcription and dictation. Terms or short phrases are added from Settings → General (min 2 chars, max 50 entries). Falls back gracefully if the CTC model can't be loaded.
...
- **Menu bar dropdown** — 268 px wide Paper-palette panel. Current status card (dark `#2E3338` background when recording/dictating, Paper background otherwise) with per-stage subtitles; the transcription stage shows a sample-count-weighted percentage ("Transcribing — 64%") that updates as each audio track completes. **Recent Meetings** list (up to 3 completed/failed jobs with click-to-open and right-click to reveal/retry/dismiss), quick actions (start dictation, open transcripts, name speakers, settings, quit), and developer-mode simulate buttons.
...
- **Name Speakers window** — 560×520 standalone scene with audio-clip playback per candidate, roster-suggestion hints, save/skip, and a 120 s dormancy-aware auto-dismiss (any text edit resets the timer, so the window only closes if no names have been entered for the full duration).
